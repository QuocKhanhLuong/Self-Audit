...
## 17. Regenerated pre-data freeze status (2026-09-18)

The invalid payload `6feede910fdcb9c9` was discarded after
`high_k_cuts_like_extra_components_void` violated the fixture contract.  The
repository had no separate fixture generator at that point; the generated JSON
was the manual source.  The authoritative pre-freeze sources are now
`configs/adapter_v1_spec_source.json` and
`configs/adapter_v1_synthetic_fixtures_source.json`, regenerated by
`scripts/regenerate_cardiac_benchmark_freeze.py`.

The corrected row preserves semantic `BVMLLMB` and derives validity
`1011111`.  Both generator and validator fail closed on every semantic/validity
pixel mismatch, invalid symbol, duplicate ID, noncanonical ordering, or
GT-shaped fixture field.  Replacement local freeze:
`cardiac-benchmark-v1-2200633e7f727d37`; scientific payload SHA-256:
`2200633e7f727d379058c11713308a4fd1dace76e8c573cb7ce4d23dba326391`.
The split, inventory, context, and spatial contracts are unchanged; real data
and physical GT isolation remain deferred.
