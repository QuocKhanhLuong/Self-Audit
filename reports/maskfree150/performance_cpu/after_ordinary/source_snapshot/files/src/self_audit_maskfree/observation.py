...
def _batch_log_mixture(
    ...
    for region in range(NUM_REGIONS):
        ...
        if not bool(mask.any()):
            continue
        region_means = means[:, region, :].unsqueeze(-1)
        region_variances = variances[:, region, :].unsqueeze(-1)
        density = _log_density_batch(
            values,
            region_means,
            region_variances,
            distribution,
            normalizers[:, region, :].unsqueeze(-1),
        )
        density = density + log_weights[:, region, :].unsqueeze(-1)
        output = torch.where(mask, torch.logsumexp(density, dim=1), output)
