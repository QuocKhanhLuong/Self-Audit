...
from .cache import SourceDataCache, default_source_cache, manifest_digest as source_manifest_digest
...
_DEFAULT_CACHE = default_source_cache()
...
def _manifest_digest(manifest: dict[str, Any]) -> str:
    """Return the complete manifest digest for an arbitrary caller mapping.

    Public loaders intentionally recompute this digest.  A caller may mutate a
    record in place without replacing the list or top-level identity fields;
    memoising by object identity would then serve source data under stale
    manifest provenance.  Frozen hot-loop callers can pass the private digest
    override accepted by the loader APIs below.
    """
    return source_manifest_digest(manifest)
    ...
    *,
    ...
    with current_progress().stage("data.decode_slice_stack", path=record["path"],
        ...
        if cache is None:
            ...
            stack = cache.get_slice_stack(
                record,
                manifest_digest_value=manifest_digest or str(manifest_id or ""),
...
def build_training_unit(
    ...
    *,
    ...
    seed: int = 42,
    cache: SourceDataCache | None = _DEFAULT_CACHE,
    _manifest_digest_value: str | None = None,
) -> TrainingUnit:
    ...
    return _build_training_unit_from_record(
        ...
        manifest_digest=(
            ...
            if cache is None
                ...
                _manifest_digest_value
                if _manifest_digest_value is not None
                else _manifest_digest(manifest)
            )
        ),
    )
...
def _build_training_unit_from_record(
    ...
    *,
    ...
    built = _build_unit_tensors(
        record,
        partition,
        image_size,
        cache=cache,
        manifest_digest=manifest_digest,
        manifest_id=str(manifest.get("manifest_id")) if manifest.get("manifest_id") is not None else None,
    )
...
class ImageOnlyDataset:
    ...
    def cache_stats(self) -> dict[str, int] | None:
        """Return source-cache counters without exposing cached arrays."""
