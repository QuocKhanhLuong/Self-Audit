"""Preregistered geometric seed falsification; image-only, no C0/C1 labels/weights.
...
CONFIG = {
    'experiment_id': 'astra-v3-r0-geometric-seeds-20260920-v1',
    'seed': 17, 'patients_per_split': 4,
    'selection': 'lowest sha256(astra-v3-r0- + patient_id) independently in original train/dev',
    'normalization_percentiles': [1, 99], 'gaussian_mm': 1.0,
    'bright_quantile': 70, 'bright_floor': 0.45,
    'lv_radius_mm': [6, 35], 'lv_max_eccentricity': 0.85,
    'lv_min_solidity': 0.75, 'lv_min_contrast': 0.08,
    'lv_max_normalized_distance_from_fov_center': 0.35,
    'winner_ratio': 1.2, 'seed_erosion_mm': 1.5,
    'myo_annulus_mm': [1.5, 5.0], 'myo_intensity_floor': 0.10,
    'rv_radius_mm': [4, 35], 'rv_max_center_distance_mm': 65.0,
    'orientation_rv_right_of_lv_mm': 2.0,
    'topology_min_dark_annulus_fraction': 0.70,
    'boundary_min_contrast': 0.16,
    'cross_slice_tolerance_mm': 3.0, 'augmentation_gamma': [0.9, 1.1],
    'arms': ['appearance_anchor', 'plus_orientation', 'plus_topology', 'plus_boundary',
             'plus_cross_slice', 'plus_augmentation', 'all_available', 'unknown_control'],
    'unavailable_cues': ['motion', 'full_cycle_temporal_consistency'],
...
def seed_volume(volume, affine, spacing, gamma=1.0):
    ...
    for z in range(shape[2]):
        x = ndi.gaussian_filter(image[:, :, z], sigma=np.asarray([CONFIG['gaussian_mm']]*2)/spacing[:2])
        ...
        for prop in props:
            ...
            if not (CONFIG['lv_radius_mm'][0] <= radius <= CONFIG['lv_radius_mm'][1]):
                continue
        ...
        lv_seed = ndi.distance_transform_edt(lv, sampling=spacing[:2]) >= CONFIG['seed_erosion_mm']
        ...
        for prop in props:
            ...
            if prop.label == label or not CONFIG['rv_radius_mm'][0] <= radius <= CONFIG['rv_radius_mm'][1]:
                continue
            if not 6 <= distance <= CONFIG['rv_max_center_distance_mm']:
                continue
        ...
        if rv_candidates:
            ...
            rv_seed = ndi.distance_transform_edt(lab == rv_label, sampling=spacing[:2]) >= CONFIG['seed_erosion_mm']
            ...
            orientation_ok = float(rv_world[0]-lv_world[0]) >= CONFIG['orientation_rv_right_of_lv_mm']
...
def cross_slice(seeds, spacing):
    ...
    for z in range(seeds.shape[2]):
        ...
        for c in (1, 2, 3):
            ...
            for j in neighbors:
                present = seeds[:, :, j] == c
                support = ndi.distance_transform_edt(~present, sampling=spacing[:2]) <= CONFIG['cross_slice_tolerance_mm'] if present.any() else present
...
def main():
    ...
    for patient, role in cohort:
        ...
        for source in images:
            ...
            if nii.header.get_xyzt_units()[0] != 'mm':
                raise ValueError('millimetre geometry required by fixed priors')
            ...
            combined = base.copy()
            for a in [orient, topo, bound, zstable, aug]:
                combined[a != base] = 255
            arms = dict(zip(CONFIG['arms'], [base, orient, topo, bound, zstable, aug, combined, np.full_like(base, 255)]))
            ...
            record = {'patient_id': patient, 'split': role, 'source_image': str(source),
                      'image_sha256': sha(target), 'shape': list(nii.shape), 'spacing': spacing.tolist(),
    ...
    receipt = {'schema': 'astra-v3-r0-frozen-v1', 'experiment_id': CONFIG['experiment_id'],
               ...
               'cue_availability': {'motion': 'NOT RUN: no full cine', 'full_cycle': 'NOT RUN: no full cine'},
               'semantic_source': 'explicit bright compact pool, surrounding darker annulus, adjacent bright pool and RAS prior; not learned or verified'}
