...
def _require_frozen_grid(manifest: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise ArtifactError("CUTS scientific runner requires a frozen approved Self-Audit baseline grid")


def _normalization_for_grid(manifest: dict[str, Any]) -> str:
    version = manifest["shared_grid"]["version"]
    return (
        SELF_AUDIT_HISTORICAL_224_NORMALIZATION_VERSION
        if version == SELF_AUDIT_HISTORICAL_224_SPATIAL_CONTRACT_VERSION
...
def run(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    config = Stage1Config(
        ...
        input_normalization=_normalization_for_grid(manifest),
    )
