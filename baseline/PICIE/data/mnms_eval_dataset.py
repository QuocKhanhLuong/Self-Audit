...
MNMS_TO_ACDC = {0: 0, 1: 3, 2: 2, 3: 1}


class EvalMnMs(data.Dataset):
    def __init__(self, root, mode='test', res=224, label=True):
        self.root = root
        self.mode = mode
        self.res = res
        self.label = label
        ...
        self._remap = np.zeros(256, dtype=np.int64)
        for src, dst in MNMS_TO_ACDC.items():
            self._remap[src] = dst

        self.slices = self._build_slice_index()

    def _build_slice_index(self):
        vol_dir = os.path.join(self.root, 'volumes')
        vol_files = sorted([f[:-4] for f in os.listdir(vol_dir) if f.endswith('.npy')])
        ...
        for vol_name in vol_files:
            ...
            if n_slices is None:
                vol = np.load(os.path.join(vol_dir, vol_name + '.npy'), mmap_mode='r')
    ...
    def _load_mask(self, vol_name):
        if vol_name not in self._masks_cache:
            ...
            raw = np.load(path).astype(np.int64)
            self._masks_cache[vol_name] = self._remap[raw]
