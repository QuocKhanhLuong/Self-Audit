...
PROFILES = ("PICIE-2D",)
NORMALIZATION_VERSION = "picie.cardiac.mri_adapter_geometric_only.v1"
...
class PICIECardiacDataset(Dataset[ImageOnlySample]):
    """Image-only dataset for PICIE.

    Reads 2.5D context stack, applies min-max normalization, then ImageNet mean/std.
    Only geometric augmentations are allowed (flip, crop).
    """

    def __init__(self, manifest: dict[str, Any], *, split: str, profile: str = "PICIE-2D", target_hw: tuple[int, int] | None = None):
    ...
    def __getitem__(self, index: int) -> ImageOnlySample:
        ...
        image = torch.from_numpy(stack) # 3 channels from context_indices
