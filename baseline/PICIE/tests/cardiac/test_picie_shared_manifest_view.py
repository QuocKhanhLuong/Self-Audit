...
from cardiac_benchmark.dataset import PICIECardiacDataset, NORMALIZATION_VERSION
from cardiac_benchmark.manifest import counts_by_split, validate_manifest


def test_picie_shared_manifest_view_preserves_identity_and_grid(tmp_path):
    manifest = shared_manifest(tmp_path, target_hw=(8, 8))
    record = next(row for row in manifest["records"] if row["slice_index"] == 0)
    ds = PICIECardiacDataset(manifest, split=record["split"], source_root=tmp_path)
...
def test_picie_rejects_grid_override(tmp_path):
    manifest = shared_manifest(tmp_path, target_hw=(8, 8))
    record = manifest["records"][0]
    with pytest.raises(SharedManifestError):
        PICIECardiacDataset(manifest, split=record["split"], source_root=tmp_path, target_hw=(16, 16))


def test_picie_manifest_rejects_missing_source_hash(tmp_path):
    manifest = shared_manifest(tmp_path, target_hw=(8, 8))
...
def test_picie_counts_by_split_uses_shared_records(tmp_path):
    manifest = shared_manifest(tmp_path, target_hw=(8, 8))
    counts = counts_by_split(manifest)
    assert sum(row["samples"] for row in counts.values()) == len(manifest["records"])


def test_picie_sa224_uses_self_audit_normalized_contract(tmp_path):
    manifest = self_audit_manifest(tmp_path)
    record = manifest["records"][0]
    ds = PICIECardiacDataset(manifest, split=record["split"], profile="PICIE-SA224", source_root=tmp_path / "images")
    sample = ds[0]
    ...
    assert sample.provenance["normalization"] == "picie.cardiac.sa224_self_audit_volume_then_fixed_affine_imagenet_v1"


def test_picie_sa224_fair_reuses_common_context(tmp_path):
    manifest = self_audit_manifest(tmp_path)
    record = manifest["records"][0]
    ds = PICIECardiacDataset(manifest, split=record["split"], profile="PICIE-SA224-FAIR", source_root=tmp_path / "images")
    sample = ds[0]
    assert tuple(sample.image.shape) == (3, 224, 224)
...
def test_picie_sa224_rejects_non_sa224_grid(tmp_path):
    manifest = shared_manifest(tmp_path, target_hw=(8, 8))
    record = manifest["records"][0]
    ds = PICIECardiacDataset(manifest, split=record["split"], profile="PICIE-SA224", source_root=tmp_path)
    with pytest.raises(SharedManifestError, match="PICIE-SA224"):
        ds[0]
