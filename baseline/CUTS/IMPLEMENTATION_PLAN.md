...
- Match whole-image field of view, orientation handling and target grid across primary comparisons. The old 224x224 template is historical; Self-Audit ACDC v3 freezes 256x256 whole-FOV bilinear values with no crop. No GT-centered cropping.
- The upstream CUTS repository has no cardiac MRI loader. Its closest NIfTI path (`src/datasets/brain_tumor_nifti.py:66-69`) applies a dataset-specific per-image min/max map to `[-1,1]`; the generic CUTS model itself performs no intensity preprocessing. Do not transplant that unrelated brain-tumor rule, and do not invent the former cardiac adapter's percentile-to-unit-interval transform.
- For Self-Audit ACDC v3, the image-only adapter must consume the checked-in source preprocessing (`scripts/preprocess_acdc.py:19-25`; `src/self_audit/data/common.py:188-204,529-546`): one 0.5/99.5 clipped, volume-wise z-score, followed by central-channel selection for CUTS-2D or endpoint-replicated stack selection for CUTS-2.5D. No additional CUTS-specific statistics are fit on train, dev, or test; the latter does not exist.
