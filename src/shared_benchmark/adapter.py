"""Frozen, topology-only implementation of ``cardiac_adapter_v1``.
...
def _assign(
    ...
    reasons.pop(component.key, None)
...
def _background(graph: RegionGraph, semantic: np.ndarray, assignments: dict[str, int], reasons: dict[str, str]) -> dict[str, str]:
    ...
    if len(seeds) != 1:
        for component in candidates:
            _void_component(component, "ambiguous_bg", reasons)
    ...
    for component in candidates:
        ...
        if component.border_contact and component.source_id == seed.source_id:
            _assign(semantic, component, BG, assignments, reasons)
    return {"BG": "resolved"}
...
def _resolve_enclosure(
    ...
    if len(pairs) == 1:
        outer, inner = pairs[0]
        _assign(semantic, outer, MYO, assignments, reasons)
        _assign(semantic, inner, LV, assignments, reasons)
        return {"MYO": "resolved", "LV": "resolved"}
    if len(pairs) > 1:
        for outer, inner in pairs:
            _void_component(outer, "ambiguous_enclosure", reasons)
            _void_component(inner, "ambiguous_enclosure", reasons)
    ...
    for component in graph.components:
        if component.key not in assignments and not component.border_contact:
            _void_component(component, "missing_enclosed_cavity", reasons)
...
def _resolve_rv(
    ...
    if len(candidates) == 1:
        _assign(semantic, candidates[0], RV, assignments, reasons)
        return {"RV": "resolved"}
    if len(candidates) > 1:
        for component in candidates:
            _void_component(component, "ambiguous_rv", reasons)
        return {"RV": "ambiguous_rv"}
    return {"RV": "unsupported_rv"}


def _assignment_trace(graph: RegionGraph, assignments: Mapping[str, int], reasons: Mapping[str, str]) -> list[dict[str, Any]]:
    return [
        {
            ...
            "reason": reasons.get(component.key, "unassigned_component"),
        }
        for component in graph.components
    ]
...
def adapt_partition(
    ...
    *,
    ...
    """Resolve one anonymous partition under the immutable v1 contract."""
    ...
    role_reasons.update(_background(graph, semantic, assignments, component_reasons))
    role_reasons.update(_resolve_enclosure(graph, semantic, assignments, component_reasons))
    role_reasons.update(_resolve_rv(graph, semantic, assignments, component_reasons))
    for component in graph.components:
    ...
    trace = _assignment_trace(graph, assignments, component_reasons)
    ...
    metadata["metadata_sha256"] = canonical_metadata_hash(metadata)
