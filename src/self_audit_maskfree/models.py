...
resolution. Down-sampling uses adaptive average pooling to ``max(1, size // 2)``
and up-sampling interpolates back to the stored skip size, so odd, non-square
and very small inputs (down to ``1x1``) are supported without padding hacks.
...
def _down(x: torch.Tensor) -> torch.Tensor:
    """Halve spatial size, never below 1 pixel."""
