...
def _regional_margin(
    ...
    Only :meth:`ObservationModel.score` is called. No candidate is refitted, and
    the number of extra score calls and each region's valid denominator are
    recorded.
    """
    ...
    for position, region in enumerate(regions):
        ...
        selected_score = model.score(fitted[selected_index], sub_view)
        ...
        for index in disagreeing:
            if report["score_calls"] >= MAX_REGION_SCORE_CALLS:
                ...
                break
            challenger_score = model.score(fitted[index], sub_view)
            report["score_calls"] += 1
...
def audit_bank(
    ...
    *,
    ...
    # Semantic validity gates first: an unresolved role contributes nothing, and
    # its pixels keep their draft label rather than being rewritten to background.
